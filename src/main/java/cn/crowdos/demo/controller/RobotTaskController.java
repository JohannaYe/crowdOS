...
//        Map<Integer,List<Integer>> assignMap = gga_i_main.getAssignMap();
    ...
    public R<Map<String,Integer>> taskSubmit(@RequestBody RobotTask robotTask){

        robotTask.setTaskId(globalComponent.getTaskId());
        globalComponent.setTaskId(globalComponent.getTaskId() + 1);
        int venueId = globalComponent.getTaskId()%18;   // 此处确定任务所处位置
        robotTask.setVenueId(venueId);
        globalComponent.getTaskList().add(robotTask);
        ...
        Map<String,Integer> taskSubmitMap = new HashMap<>();
        taskSubmitMap.put("提交任务成功",0);
        taskSubmitMap.put("taskId", robotTask.getTaskId());


        return R.success(taskSubmitMap);
    }

    @GetMapping("taskAssignAlone")
    public R<Map<String,Integer>> taskAssignAlone(@RequestBody TaskOnlyId taskIdClass){
        int taskId = taskIdClass.getTaskId();
        List<Task> taskList = crowdKernelComponent.getKernel().getTasks();
        List<Participant> taskAssginmentScheme = crowdKernelComponent.getKernel().getTaskAssignmentScheme(taskList.get(taskId));
        Participant robot = taskAssginmentScheme.get(0);
        int robotSite = ((CoordinateParticipant)robot).getVenueId();
        Map<String, Integer> taskAssignAloneMap = new HashMap<>();
        taskAssignAloneMap.put("robotId",robotSite-3);

        return R.success(taskAssignAloneMap);
    }
     ...
     * @return {@code R<Map<String,List<Integer>>>}
     */
    @GetMapping ("taskAssignByGroup")
    public R<Map<String,Integer>> taskAssign(){
//    public void taskAssign(){
//        addRobotTaskAndRobot();
//        List<Task> tasklist = (ArrayList)crowdKernelComponent.getKernel().getTasks();
//        Map<Participant,List<Task>> tasksAssignmentScheme = crowdKernelComponent.getKernel()
//                .getTaskAssignmentScheme(tasklist);
        ArrayList<Task> tasklist = (ArrayList)crowdKernelComponent.getKernel().getTasks();
        List<List<Participant>> tasksAssginmentScheme = crowdKernelComponent.getKernel()
                .getTaskAssignmentScheme(tasklist);
//        System.out.println(tasksAssginmentScheme);

        Map<String,Integer> tasksAssignMap = new HashMap<>();
        int i = 0;
        for (List<Participant> list : tasksAssginmentScheme){
            Participant robot = list.get(1);
            int robotSite = ((CoordinateParticipant)robot).getVenueId();
            String taskName = "task"+i;
            i = i+1;
            tasksAssignMap.put(taskName,robotSite-3);
        }
